...
@objc protocol CachingPlayerItemDelegate {
    ...
    @objc optional func playerItem(_ playerItem: CachingPlayerItem, didFinishDownloadingFile url: URL)

    /// Is called every time a new portion of data is received.
    @objc optional func playerItem(_ playerItem: CachingPlayerItem,
...
open class CachingPlayerItem: AVPlayerItem {
    ...
    {
        var shouldDownloadToDisk = false
        ...
        func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
            if shouldDownloadToDisk {
                _ = mediaFile?.seekToEndOfFile()
                mediaFile?.write(data)
                mediaFile?.synchronizeFile()
            } else {
                mediaData?.append(data)
            }
            processPendingRequests()
            if shouldDownloadToDisk {
                let end = mediaFile?.seekToEndOfFile()
                owner?.delegate?.playerItem?(owner!,
                                             didDownloadBytesSoFar: Int(end!),
        ...
        func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
            ...
            if shouldDownloadToDisk {
                ...
                mediaFile = FileHandle(forUpdatingAtPath: path)
            } else {
                mediaData = Data()
            }
        ...
        func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
            ...
            if shouldDownloadToDisk {
                mediaFile?.synchronizeFile()
                owner?.delegate?.playerItem?(owner!, didFinishDownloadingFile: mediaUrl!)
            } else {
        ...
        func fillInContentInformationRequest(_ contentInformationRequest: AVAssetResourceLoadingContentInformationRequest?) {
            ...
            if playingFromData {
                ...
                if shouldDownloadToDisk {
                    contentInformationRequest?.contentLength = Int64(mediaFile!.seekToEndOfFile())
                } else {
        ...
        func haveEnoughDataToFulfillRequest(_ dataRequest: AVAssetResourceLoadingDataRequest) -> Bool {
            ...
            if shouldDownloadToDisk {
                guard let eof = mediaFile?.seekToEndOfFile(),
                      Int(eof) > currentOffset else {
                    // Don't have any data at all for this request.
                    return false
                }

                let bytesToRespond = min(Int(eof) - currentOffset, requestedLength)
                mediaFile?.seek(toFileOffset: UInt64(currentOffset))
                let dataToRespond = mediaFile!.readData(ofLength: currentOffset + bytesToRespond)
                dataRequest.respond(with: dataToRespond)

                return Int(eof) >= requestedLength + requestedOffset
            } else {
    ...
    convenience init(url: URL, shouldDownloadToDisk: Bool = false) {
        self.init(url: url, customFileExtension: nil, shouldDownloadToDisk: shouldDownloadToDisk)
    }

    /// Override/append custom file extension to URL path.
    /// This is required for the player to work correctly with the intended file type.
    init(url: URL, customFileExtension: String?, shouldDownloadToDisk: Bool = false) {
        ...
        resourceLoaderDelegate.shouldDownloadToDisk = shouldDownloadToDisk
